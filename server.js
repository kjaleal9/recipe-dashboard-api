...
const recipes = require('./LocalDatabase/Recipes');
const materialClasses = require('./LocalDatabase/MaterialClasses');
const materials = require('./LocalDatabase/MAterials');
const processClasses = require('./LocalDatabase/ProcessClasses');
const equipmentRequirements = require('./LocalDatabase/RecipeEquipmentRequirements');
...
app.get('/recipes/latest', (req, res) => {
  ...
  if (enviornment === 'Local') {
    const groupedRecipes = groupRecipes(recipes.recipes);
...
app.delete('/recipes', (req, res) => {
  ...
  if (enviornment === 'Local') {
    ...
    recipes.recipes.splice(deleteIndex, 1);
    res.json(recipes);
  }
  if (enviornment === 'Production') {
...
app.get('/material-classes', (req, res) => {
  ...
  if (enviornment === 'Local') {
    res.json(materialClasses.materialClasses);
  }
  if (enviornment === 'Production') {
...
app.get('/process-classes', (req, res) => {
  ...
  if (enviornment === 'Local') {
    res.json(processClasses.processClasses);
  }
  if (enviornment === 'Production') {
...
app.get('/process-classes/required', (req, res) => {
  ...
  if (enviornment === 'Local') {
    res.json(equipmentRequirements.EquipmentRequirements);
  }
  if (enviornment === 'Production') {
    db.select(
      'ID',
